...
start.getRange("A4:H4").merge(); start.getRange("A4:H4").values=[["You only need the first tab. Replace the blue sample answers, then read the decision on the right."]];
...
start.getRange("A8:H10").values=[
  ...
  ["2","Replace blue answers","Use the listing, your agent, rent comps, and a repair estimate.",null,null,null,null,null],
...
start.getRange("A14:H17").values=[["Blue boxes","Type here","Green","Good","Yellow","Slow down","Red","Do not proceed yet"],["YES","Meets your goal","NEGOTIATE","Close, but price/terms must improve","NO","Does not work now","Version","0.3.0"],[null,null,null,null,null,null,null,null],["Important","Educational estimates only. Double-check the address, value after repairs, rent, repair cost, taxes, insurance, and loan terms before acting.",null,null,null,null,null,null]];
...
quick.getRange("A4:H4").merge(); quick.getRange("A4:H4").values=[["START HERE: Replace the BLUE sample answers. Go top to bottom. The answer appears on the right."]]; quick.getRange("A4:H4").format={fill:"#FFF2CC",font:{color:navy,bold:true,size:12},horizontalAlignment:"center"};
section(quick,"A6:C6","STEP 1 | TYPE YOUR ANSWERS IN BLUE");
quick.getRange("A7:B15").values=[["Property address","125 Sample Ave, Miami, FL"],["What is the seller asking?",285000],["What price will you offer?",250000],["What could it sell for fixed up?",360000],["What could it rent for each month?",2850],["How much will repairs cost?",42000],["Down payment % (optional)",0.2],["Loan interest % (optional)",0.0725],["Loan years (optional)",30]];
quick.getRange("C7:C15").values=[["Copy from the listing"],["Copy from the listing"],["Enter the price you want to pay"],["Ask an agent or check sold homes"],["Check similar nearby rentals"],["Ask a contractor or use your estimate"],["Leave 20% if you do not know"],["Leave 7.25% if you do not know"],["Leave 30 years if you do not know"]];
...
quick.getRange("E8").formulas=[["=IF(J22=MAX(J22:J24),\"KEEP IT AND RENT IT\",IF(J23=MAX(J22:J24),\"FIX IT AND SELL IT\",\"SELL THE CONTRACT\"))"]];
...
quick.getRange("E12").formulas=[["=IF(J22=MAX(J22:J24),'Rental'!B20,IF(J23=MAX(J22:J24),'Flip'!B16,'Property Input'!B29))"]];
...
quick.getRange("E15").formulas=[["=IF('Rental'!B20<'Property Input'!B25,\"The monthly rental profit is too low\",IF('Flip'!B16<'Property Input'!B28,\"The flip profit is too low\",\"Double-check the fixed-up value\"))"]];
quick.getRange("E16").formulas=[["=IF(B9>E10,\"Your offer is higher than the safe price\",IF('Rental'!B22<'Property Input'!B27,\"The loan payment leaves too little room\",\"Double-check repairs and monthly costs\"))"]];
quick.getRange("E17").formulas=[["=E10"]];
quick.getRange("E18").formulas=[["=IF(B9>E10,\"Offer no more than \"&TEXT(E10,\"$#,##0\")&\", or ask the seller to cover \"&TEXT(B9-E10,\"$#,##0\")&\" of the gap.\",\"Before offering, double-check the fixed-up value, rent, repairs, taxes, insurance, and loan.\")"]];
...
const xlsx=await SpreadsheetFile.exportXlsx(wb); await xlsx.save(`${outDir}/DealCheck-Pro-Easy-Start-v0.3.xlsx`);
console.log(JSON.stringify({output:`${outDir}/DealCheck-Pro-Easy-Start-v0.3.xlsx`,inspect:inspect.ndjson,errors:errors.ndjson},null,2));
